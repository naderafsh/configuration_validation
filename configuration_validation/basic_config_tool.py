...
def main():
    ...
    master_path = find_one_master()
    filename = export_config_validation()
    with open(filename, 'w') as f:
        f.write('\n' + master_path + '\n')
        gblv_data = extract_data.main(master_path, filter_variables)
...
def export_config_validation():
    ...
    filename = output_path + '\\btoutput\\Config_Validation-{0}.csv'.format(strftime("%Y%m%d%H%M%S"))
    return filename
